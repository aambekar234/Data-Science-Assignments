...
def bfs(graph, root, max_depth):
    ...
    while q:
        ...
        for neighbor in graph.neighbors(parent):
            if neighbor in visited and (node2distances[neighbor] == node2distances[parent]+1):
                    node2parents[neighbor].append(parent)
                    
                    node2num_paths[neighbor] += 1
            elif neighbor not in visited and (node2distances[parent] < max_depth):
                    visited.add(neighbor)
                    node2num_paths[neighbor] = 1
                    node2distances[neighbor]=node2distances[parent]+1 
                    q.append(neighbor)
                    node2parents[neighbor].append(parent)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    sorted_data = sorted(node2distances.items(), key=lambda x: (-x[1]))
    for k,v in sorted_data:
        if(node2num_paths[k] > 1):
            n = 1/node2num_paths[k]
            for element in node2parents[k]:
                nodes[element] = nodes[element]+n
                nodes[k] = n  
        elif(node2num_paths[k] == 1):
            for parent in node2parents[k]:
              nodes[parent] = nodes[parent]+nodes[k]
...
def approximate_betweenness(graph, max_depth):
    ...
    for i in graph.nodes():
        ...
        tmp = bottom_up(i, node2distances, node2num_paths, node2parents)
...
def partition_girvan_newman(graph, max_depth):
    ...
    gc = graph.copy()
    components = [x for x in nx.connected_component_subgraphs(gc)]
    
    apb = approximate_betweenness(graph,max_depth)
    result = sorted(apb.items(),key=lambda x:(-x[1], x[0]))
    
    i = 0
    while len(components) == 1:
        gc.remove_edge( *(result[i][0]) )
        i += 1
        components = [x for x in nx.connected_component_subgraphs(gc)]
    return components
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    cut_st = cut(S, T, graph)
    volume_s = volume(S, graph)
    volume_t = volume(T, graph)

    ans = (cut_st / float(volume_s) ) + (cut_st / float(volume_t) )
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
        ...
        [comp1, comp2] = partition_girvan_newman(graph, i)    
        nc = norm_cut(comp1, comp2, graph)
        tmp = tuple((i, nc))
        ans.append(tmp)
...
def make_training_graph(graph, test_node, n):
    ...
    """
    ###TODO
    nbrs = sorted(graph.neighbors(test_node))
    gc = graph.copy()
    
    for i in range(n):
        gc.remove_edge(test_node, nbrs[i])
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            ...
            scores.append(((node, n), len(nbrs & nbrs2) / len(nbrs | nbrs2)))
